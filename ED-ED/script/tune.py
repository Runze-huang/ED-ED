...
class MidiPreprocessor:
    ...
    def __init__(self, 
                 classes,    #包含的音乐类型，jazz和jop
                 pickle_store_folder, #以pickle形式保存
                 note_num = 17,
                 longest = 100,
                 low_crop=55,         #低音下限          
                 high_crop=89,        #高音上限  
                 num_notes=128,       #音符one_hot数
                 smallest_note=16,    #最小采样16分音符
                 max_velocity=127,    #音符力度上限
                 input_length=32,     #输入模型的长度
                 test_fraction=0.01):     #测试集所占的比例
    ...
    def load_rolls(self, path, save_folder, save_preprocessed_midi):
        ...
        length_list = []
        try:
            mid = pretty_midi.PrettyMIDI(path)  #读取midi文件
        ...
        """
        #unit_time=0.125
        quarter_note_length = 1. / (tempo/60.)    #一个四分音符的时值，tempo表示一分钟内弹奏的四分音符的数量
        unit_time = quarter_note_length * 4. / self.smallest_note  #这里是0.125
        #没有和弦音轨则跳过这首歌
        ...
        for note in notes: 
            ...
            #print(number, "  ",now_time - note.start)
            #print (now_time - note.start ,num1 * unit_time )
