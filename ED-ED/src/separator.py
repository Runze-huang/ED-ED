...
"""
以列表形式保存每一首曲子的矩阵，每一首曲子的shape为（bars_nums, 32, 130）
"""
class MidiSeparator:
            ...
            for name in files:   
                ...
                if _name.endswith('.mid') or _name.endswith('.midi'):
                    ...
                    if pitch is not None :
                        ...
                        '''
                        file_chord = os.path.join(path,"chord")
                        if not os.path.isdir(file_chord): 
                              os.mkdir(file_chord)
                        '''
    ...
    def save_data(self, pitch, rhythm,pitch_pos, path, i, name):
        ...
        '''
        if self.save_reconstructed == "true":
            #recon = recon.view(-1,recon.size(-1)).contiguous()
            reconstruct = mf._sampling(recon)
            reconstruct = np.array(reconstruct.cpu().detach().numpy())
            reconstruct = mf.modify_pianoroll_dimentions(reconstruct,self.low_crop,self.high_crop,"add")
            mf.numpy_to_midi(reconstruct, 120, midi_path ,_name, self.smallest_note)
        '''
    ...
    def load_rolls(self, path, name, save_folder):
        ...
        long_list = []
        try:
            mid = pretty_midi.PrettyMIDI(path + name)  #读取midi文件
