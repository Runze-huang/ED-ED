                ...
                if _name.endswith('.mid') or _name.endswith('.midi'):
                    ...
                    for i, c in enumerate(self.classes):
                        if c.lower() in shortpath.lower():   
                            ...
                            print(infor[0])
                            print(length[0])
                            for i in range(source[0].shape[0]):
                                #if i < 30:
                                   print(source[0][i])
                            '''
                            #batch = torch.from_numpy(X)[:3]
                            #batch = batch.view(-1, batch.size(-1)).max(-1)[1]
                            #count = torch.bincount(batch)[7:41]
                            #print(count)
                            #return
